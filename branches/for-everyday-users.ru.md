...
Tier 2: CLI Agent  (готов выучить немного command line; автоматизация повседневных потоков)
   ↓
Tier 3: Local LLM  (конфиденциальные данные, экономия, offline)
```
...
Официальный интерфейс Anthropic. Лучше для длинных текстов, глубоких дискуссий и сложных вопросов — стиль ответов более сдержанный, меньше склонен к hallucination.
...
>
> Нужно подробное сравнение? См. [`resources/cli-agents-guide.ru.md`](../resources/cli-agents-guide.ru.md) — семь основных CLI agents бок-о-бок, рекомендации по сценариям, частые ловушки, реальные настройки.
>
...
>
> Хочешь подвязать CLI agent к Notion / Obsidian / Excel / Google docs / и т. д.? См. [`resources/mcp-skills-catalog.ru.md`](../resources/mcp-skills-catalog.ru.md) — 62 MCP server / Skill, сгруппированных по сценариям.
...
**Лучше всего для**: тех, кто разворачивает инструменты локально; людей, не желающих привязки к одному поставщику; всех, кто переключается между несколькими LLM.
...
★ 170k+ — одна команда — и локальный LLM работает. Применяй, когда конфиденциальные данные (медкарты, контракты, семейные разговоры) не должны покидать машину. См. [Этап 1 — Local LLM](../stages/01-llm-basics.ru.md).
...
5% апгрейдятся до Tier 2–3: реальные потребности автоматизации (например, делаешь одно и то же 100 раз в день) или конфиденциальные данные, которым нельзя в облако.

...
После установки CLI и первых задач следующий вопрос: **как сделать CLI консистентным, повторяемым, переиспользуемым?** Эта остановка покрывает workflow-паттерны — превращая «я перепечатываю тот же промпт каждый раз» в «настроил один раз, CLI делает правильно автоматически».

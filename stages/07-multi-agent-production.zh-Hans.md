...
8. [**jipin-ai/agent-pipeline-engine**](https://github.com/jipin-ai/agent-pipeline-engine)（新）— 多 Agent 协同的可信方向最小实现：SQLite 状态机 + 证据门禁（不信 Agent 自报，强制 git commit / 测试报告 / SHA256）+ 审计链 + 人类任意点介入。398 行纯标准库、零依赖。生产实证：证据门禁用活体检查当场拦截 Agent 拿身份名片冒充部署的假完成。配套日更实战连载 [54集](https://jipin-ai.github.io/)。

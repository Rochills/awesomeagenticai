...
8. [**stablyai/orca**](https://github.com/stablyai/orca)（★ 57k+、MIT）— 目前規模最大的「**多 agent 同時跑**」桌面環境：一個 prompt 可以同時發給多個 agent，每個跑在自己的 git worktree 裡，跑完並排比較、挑一個 merge。它把 Codex / Claude Code / OpenCode / Pi 當成可替換的後端，所以真正的看點不是它支援誰，而是**當五個 agent 同時在改同一份 repo，介面要解決哪些問題**：隔離、比較、審查，以及「agent 做完了怎麼通知你」。附 iOS app，可以在手機上盯進度。
9. [**yc-software/qm**](https://github.com/yc-software/qm)（★ 14k+、MIT）— Y Combinator 自己開源的 **quartermaster**，跑在 Slack 與 web 上。跟 orca 的分工不同：orca 處理「一個人開很多 agent」，qm 處理「**一整間公司共用 agent**」。每個員工有自己隔離的 workspace（獨立的 memory、檔案、金鑰視野、權限、排程、沙箱），同時又能在 channel 與 project 裡協作；harness 與模型都可抽換（Pi / OpenCode / Codex / Claude Code 共用同一個核心），所以部署不會綁死單一廠商。想看「**組織層級的 agent 權限與治理**長什麼樣」，這是目前少數能直接打開來讀的實作。
...
| | [AMAP-ML/LongHorizon-Harness](https://github.com/AMAP-ML/LongHorizon-Harness) | ⭐⭐⭐ | 想看「驗證那一格」在真實專案裡長什麼樣 | 把長任務拆成 Manager / Executor / **Auditor** 三個角色，Executor 每輪用新 context，Auditor 獨立檢查後才寫進持久 state——就是上面那張圖裡「檢查對不對」那一格的實作。包在 Claude Code / Codex 外層，不用自己重寫 agent loop。**很新**：2026-08-04 建立、2 位 contributor，還沒有長期維護紀錄。★ 1.4k+、MIT |

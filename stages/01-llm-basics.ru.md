...
**Без API-комиссий, работает на твоей машине**: используй Ollama, чтобы pull'нуть мелкую модель (рекомендую `llama3.2:3b` или `qwen2.5:3b`), обращайся через OpenAI-совместимый API.
```bash

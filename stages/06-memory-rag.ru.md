...
Читая этот этап, держи в голове два вопроса: какие сценарии плохо подходят для RAG? Какие сценарии подходят, но требуют большего, чем базовый RAG?
...
Для первого RAG-приложения не стартуй с хитрого splitter'а. Документация LangChain рекомендует стартовать с `RecursiveCharacterTextSplitter` для большинства сценариев, потом использовать retrieval-результаты, чтобы решить, менять ли стратегию.

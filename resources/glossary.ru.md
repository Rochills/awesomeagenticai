...
GPT, Claude, Gemini — модели, принимающие текст на входе и выдающие текст на выходе. Базовый API-вызов stateless: модель видит только переданный контекст. Веб-доступ, память прошлых разговоров и tools добавляются продуктом, agent loop или внешним слоем.
...
Максимум токенов, которые LLM «видит» за один вызов. У современных моделей это обычно десятки, сотни тысяч или больше токенов, в зависимости от провайдера и модели. **Больше не всегда лучше** — на длинном контексте качество retrieval и позиционирование фактов всё равно критичны.
...
Паттерн, где модель просит рассуждать пошагово перед финальным ответом. Может повышать качество на сложных reasoning-задачах, но стоит дороже по токенам и не должен автоматически раскрывать длинное внутреннее рассуждение пользователю. Часто достаточно попросить краткое обоснование или проверяемые шаги.
...
«Сначала найди, потом генерируй». Поток: вопрос пользователя → embedding-поиск top-K релевантных кусков → добавляешь эти K кусков в промпт → LLM отвечает. **Помогает с двумя проблемами: у модели нет твоих приватных данных в весах, а знания модели могут быть устаревшими.**
...
Запусти semantic search и keyword search вместе, слей и переранжируй. Часто даёт лучший recall, чем каждый подход по отдельности. Для production-grade RAG это сильный baseline, но качество всё равно проверяют на реальных запросах.
...
Прогон тестового набора по твоему agent'у с количественной оценкой accuracy / latency / cost. Для production-agent eval играет роль regression-тестов, но не заменяет мониторинг, human review и доменные проверки. Распространённое: promptfoo, LangSmith, langfuse evals.
...
LLM возвращает токены по мере генерации вместо ожидания полного ответа. Часто лучший UX для interactive apps; технически через SSE или chunked transfer. Trade-offs: клиент должен обрабатывать partial responses; парсинг structured output и tool calls часто требует дождаться полного сообщения.
...
LLM выдаёт правдоподобный, но неверный ответ — придумывает API, фабрикует числа или неверно связывает факты. Production-agent снижает этот риск через RAG, structured output, eval, guardrails и human review там, где цена ошибки высока.

### Frontier Model

Модели верхнего класса у крупных провайдеров на текущий момент. Frontier-модели используют для тяжёлых рассуждений, сложного кода и длинного контекста; дешёвые мелкие модели — для простой классификации, извлечения и перевода, чтобы экономить.
